...
void UART0_Send_Frame(unsigned char Length) {
    if (Uart0.fd >= 0) write(Uart0.fd, Radar.UART_Frame0, Length);
}

void UART1_Send_Frame(unsigned char Length) {
    if (Uart1.fd >= 0) write(Uart1.fd, Radar.UART_Frame1, Length);
}

void UART2_Send_Frame(unsigned char Length) {
    if (Uart2.fd >= 0) write(Uart2.fd, Radar.UART_Frame2, Length);
}

void UART3_Send_Frame(unsigned char Length) {
    if (Uart3.fd >= 0) write(Uart3.fd, Radar.UART_Frame3, Length);
}
...
void Radar_Innit_SPI(void)
{
    ...
    // Pl_Spi0 -> LMK04832
    if (SPI_Init_Linux(&Pl_Spi0, "/dev/spidev1.0", lmk_lmx_mode, lmk_lmx_speed) != XST_SUCCESS) {
        all_spi_ok = 0;
    }

    // Pl_Spi1 -> LMX2820 PLL1
    if (SPI_Init_Linux(&Pl_Spi1, "/dev/spidev2.0", lmk_lmx_mode, lmk_lmx_speed) != XST_SUCCESS) {
        all_spi_ok = 0;
    }

    // Pl_Spi2 -> LMX2820 PLL2
    if (SPI_Init_Linux(&Pl_Spi2, "/dev/spidev3.0", lmk_lmx_mode, lmk_lmx_speed) != XST_SUCCESS) {
        all_spi_ok = 0;
    }
    
    // Pl_Spi3 -> ADF4360
    if (SPI_Init_Linux(&Pl_Spi3, "/dev/spidev4.0", adf_mode, adf_speed) != XST_SUCCESS) {
        all_spi_ok = 0;
    }
    
    // Pl_Spi4 -> ADF4159
    if (SPI_Init_Linux(&Pl_Spi4, "/dev/spidev5.0", adf_mode, adf_speed) != XST_SUCCESS) {
        all_spi_ok = 0;
    }
...
void Radar_Innit_GPIO(void)
{
	...
	{
		// cau hinh DIR
		XGpio_SetDataDirection(&GPIO_ENDFRAME_IRQ, 1, 0x1);
		xil_printf("GPIO_ENDFRAME_IRQ ok\r\n");
	}

	// do GPIO_ENDFRAME_IRQ1: đang được dùng để sử dụng ngắt, nên không được cấu hình nữa
	// Status0 = XGpio_Initialize(&GPIO_ENDFRAME_IRQ1, ENDFRAME_IRQ1_PORT_ID);
	// if (Status0 == XST_SUCCESS)
	// {
	// 	// cau hinh DIR
...
void Radar_Check_motor(void)
{
	if(Radar.Beta_SW % 150 == 0)
	{
		...
		if(PC8.Alpha_A0 == PC8.Alpha_A1) PC8.Alpha_Stop = 1; else PC8.Alpha_Stop = 0;

		if(PC8.Beta_Calib == 0)
		{
			PC8.Beta_A1	= PC8.Beta_A0;
			PC8.Beta_A0	= PC8.Beta_ENC;
			if(PC8.Beta_A0 == PC8.Beta_A1) PC8.Beta_Stop = 1; else PC8.Beta_Stop = 0;
		}
		else if(PC8.Beta_Calib == 2)
		{
			if(PC8.Calib_counter < 10) PC8.Calib_counter ++;
