...
int LMK04832_ReadRegister(LinuxSpiDevice *SpiInstance, uint16_t address) {
    ...
    tx_buffer[2] = 0x00; // Gửi một byte rỗng để tạo xung clock cho chip gửi dữ liệu về
...
void Start_Test_System(void)
{
	...
	*/
	//	xil_printf("Configuring CLK104 LMK and LMX devices\r\n");
		...
		// // gửi lênh tắt mode 3wire tại thanh ghi 0x00
		// sendbuffer[0] = (u8)(0x00);
		// sendbuffer[1] = (u8)(0x00);
		// sendbuffer[2] = (u8)(0x10);
		...
		// // gửi lênh tắt mode 3wire tại thanh ghi 0x00
		// sendbuffer[0] = (u8)(0x01);
		// sendbuffer[1] = (u8)(0x6E);
		// sendbuffer[2] = (u8)(0x3B);
		...
		// for(i=0; i< 125 ; i++)
		// {
		// 	current_address = LMK04832_registers[i].address;
    	// 	value_to_write  = (u8)(LMK04832_registers[i].value);

		// 	// --- BƯỚC 2: ĐỌC LẠI ĐỂ KIỂM TRA ---
		// 	//usleep(20000); 
		// 	read_back_value = LMK04832_ReadRegister(&Pl_Spi0, current_address);
		// 	//xil_printf("Addr[0x%04X]: W=0x%02X, R=0x%02X\n\r", current_address, value_to_write, (uint8_t)read_back_value);
		// 	//--- BƯỚC 3: SO SÁNH VÀ THÔNG BÁO ---
		// 	if( (current_address ==0x0000)||
		// 		(current_address ==0x016E)||
		// 		(current_address ==0x0183))
		// 	{

		// 	}
		// 	else
		// 	{
		// 		if (read_back_value < 0) 
		// 		{ 
		// 			xil_printf("=> read Fail\n\r"); 
		// 		} 
		// 		else if ((uint8_t)read_back_value != value_to_write) 
		// 		{ 
		// 			xil_printf("=> Fail Addr[0x%04X]: W=0x%02X, R=0x%02X\n\r", current_address, value_to_write, (uint8_t)read_back_value);
		// 		} 
		// 		else 
		// 		{
		...
		for(i=0; i< 123 ; i++)
		{
			...
			// --- BƯỚC 2: ĐỌC LẠI ĐỂ KIỂM TRA ---
			usleep(20000); 
			// read_back_value2 = LMX2820_ReadRegister(&Pl_Spi1, current_address2);

			// // --- BƯỚC 3: SO SÁNH VÀ THÔNG BÁO ---
			// if( (current_address2 ==0x0047)||
			// 	(current_address2 ==0x0048)||
			// 	(current_address2 ==0x0049)||
			// 	(current_address2 ==0x004A)||
			// 	(current_address2 ==0x004B)||
			// 	(current_address2 ==0x0071)||
			// 	(current_address2 ==0x0074)||
			// 	(current_address2 ==0x0077))
			// {

			// }
			// else
			// {
			// 	if (read_back_value2 < 0) 
			// 	{ 
			// 		xil_printf("=> read Fail\n\r"); 
			// 	} 
			// 	else if (read_back_value2 != value_to_write2) 
			// 	{ 
			// 		xil_printf("=> Fail Addr[0x%04X]: W=0x%04X, R=0x%04X\n\r", current_address2, value_to_write2, (uint8_t)read_back_value2);
			// 	} 
			// 	else 
			// 	{
		...
		for(i=0; i< 123 ; i++)
		{
			...
			// --- BƯỚC 2: ĐỌC LẠI ĐỂ KIỂM TRA ---
			usleep(20000); 
			// read_back_value2 = LMX2820_ReadRegister(&Pl_Spi2, current_address2);
	...
	*/
	...
	//    sleep(2);
		...
		Status = metal_device_open("platform", "a0000000.usp_rf_data_converter", &device);
		if (Status != 0) {
