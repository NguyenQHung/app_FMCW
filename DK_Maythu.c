...
{
	if(AFC1.Auto_Start == 0) AFC1.Auto_Counter++;
...
void Radar_taogia_Encoder(void)
{
	...
	if(Radar.Beta_SW_en >= 4)
	{
		Radar.Beta_SW_en = 0;
		Radar.Beta_SW++;
		if(Radar.Beta_SW >= 3600) Radar.Beta_SW = 0;
	}
}
...
void Radar_Read_Encoder(void)
{
	...
	if(Radar.Beta_HW >= 3600) Radar.Beta_HW=0;
	IP_Set_Beta_in(Radar.Beta_HW);

	PC8.Beta_Angle = Radar.Beta_HW;
	if(PC8.Beta_Angle>=3600) PC8.Beta_Angle=0;
	...
	if(Radar.Errori > 1) Radar.ENC_Error++;
	...
	if(Radar.Beta_HW > 10) Radar.Debug_ENC =0;

}

void Radar_Setup_Header(void)
{
	if(Radar.Range<=1) {Radar.Sample=356;		Radar.ADC_Freq0=40; }
	else if(Radar.Range==2) {Radar.Sample=1124;	Radar.ADC_Freq0=40; }
	else if(Radar.Range==3) {Radar.Sample=2148;	Radar.ADC_Freq0=40; }
	else if(Radar.Range==4) {Radar.Sample=4196;	Radar.ADC_Freq0=40; }
	else if(Radar.Range==5) {Radar.Sample=4196;	Radar.ADC_Freq0=10; }
	else if(Radar.Range==6) {Radar.Sample=4196;	Radar.ADC_Freq0=10; }
	else if(Radar.Range==7) {Radar.Sample=4196;	Radar.ADC_Freq0=10; }
	...
	if(Radar.Compass_PC2_EN == 0)
	{
		if(Radar.Compass_PC2_Fix < 50)
		{
			...
			{
			Frame.Radar_Azimuth = 3550 + Radar.Radar_Azimuth + Radar.Compass_PC2_Fix;
			}
		}
		else if(Radar.Compass_PC2_Fix >= 50)
		{
			...
			if(Frame.Radar_Azimuth>= 3600) Frame.Radar_Azimuth = Frame.Radar_Azimuth - 3600;
		}
	}
...
{
	memcpy(&Radar_Frame1[0]  , &Fix_Frame, 100);
}
...
//	if(PC8.code_data == 1)
//	{
//		if(PC8.Control_View_SS == 9) // Video_Pulse16_Addr // Video_Code163_Addr
//		{
//			Linux_memcpy(&Radar_Frame_M[0], Video_Pulse16_Addr, 24000);
//		}
...
//			memcpy(&Radar_Frame1[3300], &Radar_Frame_M[4216], 18160);
//		}
//		else if(PC8.code_data == 0)
//		{
