...
// XScuGic 		ZYNQ_INTR;				// ZYNQ interrupt device
  ...
  XGpio             GPIO_TX_PREAMBLEIN;
  XGpio             GPIO_RX_PREAMBLEOUT;
...
// Giúp trình biên dịch biết các hàm này tồn tại trước khi nó thấy định nghĩa của chúng.
...
int start_application(uint16_t local_port) {
    ...
    if (bind(udp_conn.sockfd, (const struct sockaddr *)&server_addr, sizeof(server_addr)) < 0) {
...
int main(void)
{
	...
	// In thông tin cấu hình mạng (trên Linux, IP được cấu hình tĩnh bởi hệ điều hành)
    //xil_printf("Cau hinh mang can duoc thiet lap tinh trong /etc/network/interfaces\n");
	...
	// else
	// {
	// 	xil_printf("pthread_create(&udp_tid, NULL, udp_control_thread, NULL) => done \r\n");
	// }
	...
	{
		xil_printf("pthread_create(&can_thread_id, NULL, can_receiver_thread, NULL) => done \r\n");
	}
