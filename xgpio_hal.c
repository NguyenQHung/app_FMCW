...
static GpiodMap gpiod_hal_table[] = {
    // {&InstanceGoc,      "Tên chip", chip, {0}, {0}, Rộng CH1, Rộng CH2,
    // false},
    ...
    {&GPIO_SYNC_CMAC, "gpiochip1", NULL, {0}, {0}, 8, 0, false}};
#define GPIOD_HAL_TABLE_SIZE                                                   \
  (sizeof(gpiod_hal_table) / sizeof(gpiod_hal_table[0]))
...
int XGpio_Initialize(XGpio *InstancePtr, uint32_t Chip_index) {
  ...
  if (map->is_initialized)
    return XST_SUCCESS; // Đã khởi tạo rồi

  map->chip = gpiod_chip_open_by_name(map->chip_name);
  if (!map->chip) { /* ... xử lý lỗi ... */
    return XST_FAILURE;
  }

  if (map->width_ch1 > 0) {
    int *offsets = malloc(sizeof(int) * map->width_ch1);
    for (int i = 0; i < map->width_ch1; i++)
      offsets[i] = i;
    if (gpiod_chip_get_lines(map->chip, offsets, map->width_ch1,
                             &map->lines_ch1) < 0) { /* ... xử lý lỗi ... */
      free(offsets);
      return XST_FAILURE;
    }
    free(offsets);
  ...
  if (map->width_ch2 > 0) {
    ...
    for (int i = 0; i < map->width_ch2; i++)
      offsets[i] = map->width_ch1 + i;
    if (gpiod_chip_get_lines(map->chip, offsets, map->width_ch2,
                             &map->lines_ch2) < 0) { /* ... xử lý lỗi ... */
      free(offsets);
      return XST_FAILURE;
    }
    free(offsets);
...
void XGpio_SetDataDirection(XGpio *InstancePtr, int Channel,
  ...
  if (!map || !map->is_initialized) {
    fprintf(stderr,
            "[HAL ERROR] SetDataDirection: Instance chưa được khởi tạo.\n");
    return;
  }

  // Xác định đúng kênh
  struct gpiod_line_bulk *lines =
      (Channel == 1) ? &map->lines_ch1 : &map->lines_ch2;
  int width = (Channel == 1) ? map->width_ch1 : map->width_ch2;
  if (width == 0)
    return; // Bỏ qua nếu kênh không có chân nào
  ...
  if (DirectionMask != 0) {
    ...
    if (gpiod_line_request_bulk_input(lines, GPIO_CONSUMER) < 0) {
      ...
      fprintf(stderr,
              "[HAL ERROR] gpiod_line_request_bulk_input thất bại trên chip "
              "'%s' (Kênh %d): %d pin",
              map->chip_name, Channel, width);
      perror(NULL);
    } else {
      // Thông báo thành công (tùy chọn, có thể xóa đi nếu không muốn quá nhiều
      // log)
    ...
    // --- CẤU HÌNH LÀM OUTPUT ---
    // Tham số cuối là giá trị mặc định, NULL có nghĩa là không thay đổi giá trị
    // hiện tại.
    if (gpiod_line_request_bulk_output(lines, GPIO_CONSUMER, NULL) < 0) {
      // Nếu request thất bại, báo lỗi chi tiết
      fprintf(stderr,
              "[HAL ERROR] gpiod_line_request_bulk_output thất bại trên chip "
              "'%s' (Kênh %d):  %d pin",
              map->chip_name, Channel, width);
...
void XGpio_DiscreteWrite(XGpio *InstancePtr, int Channel, uint32_t Data) {
  ...
  if (gpiod_line_set_value_bulk(lines, values) < 0) {
    ...
    //  Nếu có lỗi, in ra thông báo chi tiết, bao gồm cả tên chip và lỗi hệ
    //  thống
    fprintf(stderr,
            "[HAL ERROR] gpiod_line_set_value_bulk thất bại trên chip '%s' "
            "(Kênh %d): ",
            map->chip_name, Channel);
    perror(NULL); // perror(NULL) sẽ chỉ in ra thông báo lỗi hệ thống (ví dụ:
...
uint32_t XGpio_DiscreteRead(XGpio *InstancePtr, int Channel) {
  ...
  int *values = malloc(sizeof(int) * width);
  if (gpiod_line_get_value_bulk(lines, values) < 0) { /* ... xử lý lỗi ... */
    free(values);
    return 0;
  }
...
void XGpio_CleanupAll(void) {
  for (int i = 0; i < GPIOD_HAL_TABLE_SIZE; i++) {
    if (gpiod_hal_table[i].is_initialized) {
      if (gpiod_hal_table[i].width_ch1 > 0)
        gpiod_line_release_bulk(&gpiod_hal_table[i].lines_ch1);
      if (gpiod_hal_table[i].width_ch2 > 0)
        gpiod_line_release_bulk(&gpiod_hal_table[i].lines_ch2);
