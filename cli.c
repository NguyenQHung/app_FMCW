...
void enableRawMode() {
    ...
    atexit(disableRawMode); // Tự động khôi phục khi thoát app

    struct termios raw = orig_termios;
    // Tắt echo (hiện phím), tắt chế độ nấu (canonical) để nhận từng phím
    raw.c_lflag &= ~(ECHO | ICANON | ISIG | IEXTEN);
    // Tắt các xử lý đầu vào đặc biệt
...
/**
 * Thay thế inbyte() của Xilinx bằng getchar() chuẩn Linux
 */
char inbyte(void) {
    char c;
...
char* getAndParseInput(void) {
    ...
    // Đảm bảo terminal đang ở chế độ Raw để nhận phím điều hướng
    enableRawMode();

    while(1) {
        temp = inbyte();
        
        if (temp == 0x0D || temp == 0x0A || temp == 0) { // Enter
            printf("\r\n");
            break; 
        } else if (temp == 0x7F || temp == 0x08) { // Backspace
            if (i > 0) {
                i--;
                printf("\b \b");
                fflush(stdout);
            }
        } else if (temp == 0x1B) { // Escape Sequence (Phím mũi tên)
            char seq[3];
    ...
    line[i] = '\0';
    disableRawMode(); // Trả lại chế độ bình thường sau khi nhập xong

    strncpy(lineOld, line, LINEMAX);
    
    // Logic strtok giữ nguyên như cũ
    cmdPtr = strtok(line, seps);
    numCmdVals = 0;
    char *cmdValPtr;
    for(int j=0; j<MAX_CMD_VALS; j++) {
        cmdValPtr = strtok(NULL, seps);
        if(!cmdValPtr) break;
        if((cmdValPtr[0]=='0') && (tolower(cmdValPtr[1])=='x')) {
            cmdVals[j] = (u32)strtoul(cmdValPtr, NULL, 16);
            numCmdVals++;
        } else {
            cmdVals[j] = (u32)atoi(cmdValPtr);
            numCmdVals++;
        }
    }
    return cmdPtr;
