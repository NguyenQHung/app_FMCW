...
#define ZYNQMP_GPIO_CHIP_NAME "gpiochip11"
...
int XGpioPs_CfgInitialize(XGpioPs *InstancePtr, XGpioPs_Config *ConfigPtr, unsigned long EffectiveAddr) {
    if (ps_gpio_state.is_initialized) return XST_SUCCESS;
...
void XGpioPs_SetDirectionPin(XGpioPs* InstancePtr, int Pin, int Direction) {
    if (!ps_gpio_state.is_initialized || Pin < 0 || Pin >= ZYNQMP_GPIO_NUM_PINS) return;
    ...
    if (!gpiod_line_is_requested(ps_gpio_state.lines[Pin]) || 
        gpiod_line_direction(ps_gpio_state.lines[Pin]) != (Direction ? GPIOD_LINE_DIRECTION_OUTPUT : GPIOD_LINE_DIRECTION_INPUT)) 
    {
        ...
        if (Direction == 1) { // Output
            ...
            ret = gpiod_line_request_output(ps_gpio_state.lines[Pin], GPIO_CONSUMER, 0);
        } else { // Input
...
/**
 * @brief Bật/tắt buffer đầu ra. Trong libgpiod, nó tương đương với SetDirection.
 */
...
/**
 * @brief Ghi giá trị (0 hoặc 1) vào một chân output, sau đó đọc lại để xác nhận.
 */
void XGpioPs_WritePin(XGpioPs* InstancePtr, int Pin, int State) {
    if (ps_gpio_state.lines[Pin] != NULL && gpiod_line_is_requested(ps_gpio_state.lines[Pin])) {
